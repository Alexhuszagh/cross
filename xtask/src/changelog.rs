...
fn read_changelog(root: &Path) -> cross::Result<(String, Changes, String)> {
    ...
    let next_index = lines
        .iter()
        .position(|x| x.trim() == "<!-- next-header -->")
        .ok_or(eyre::eyre!(
            "could not find \"<!-- next-header -->\" section"
        ))?;
    let (header, rest) = lines.split_at(next_index);
    let rest = &rest[1..];
    let last_index = rest
        .iter()
        .position(|x| x.trim() == "<!-- last-header -->")
        .ok_or(eyre::eyre!(
            "could not find \"<!-- last-header -->\" section"
        ))?;
    let (unreleased, footer) = rest.split_at(last_index);
    let footer = &footer[1..];
...
fn move_changes(root: &Path, version: &str) -> cross::Result<()> {
    // move all files to the denoted version release
    let src = root.join(".changes");
    let dst = src.join(Path::new(version));
...
fn build_changelog_from_dir(root: &Path, changes_dir: &Path) -> cross::Result<String> {
...
mod tests {
    ...
    #[test]
    fn test_build_changelog() -> cross::Result<()> {
        let mut msg_info = MessageInfo::default();
        let root = project_dir(&mut msg_info)?;
        let changes_dir = root.join(".changes").join("template");

        let output = build_changelog_from_dir(&changes_dir, &changes_dir)?;
